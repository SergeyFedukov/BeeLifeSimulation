...
public class Map
{
    ...
    public bool TryCreate(Cell cellPrefub, float hiveCellSize)
    {
        ...
        AddAdjacentÑells(_hiveCells);
        AddAdjacentÑells(_plantCells);
        return true;
    }
    ...
    private void AddAdjacentÑells(Cell[,] cells)
    {
        int width = cells.GetUpperBound(0) + 1;
        int height = cells.Length / width;
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                cells[i,j].TryAddAdjacentÑells(topCell, bottomCell, leftCell, rightCell);
            }
        }
    }
}